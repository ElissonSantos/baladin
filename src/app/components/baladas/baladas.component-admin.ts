...
import { BaladasService } from "~/app/services/baladas.service";
...
export class BaladasAdminComponent implements OnInit {
    ...
    allEventos: Array<EventoResouce> = [];
    eventosShow: Array<EventoResouce> = [];
    isAdmin: boolean;
    ...
    create() {
        this.routerExtensions.navigate(["/createEvento"]);
    }
    ...
    eventosExibir() {
        ...
        this.allEventos.forEach(evento => {
            const dia: Date = moment(evento.data).toDate();
            if (dia.getDate() === this.diaSelecionado.getDate()) {
    ...
    alterarSenha() {
        this.routerExtensions.navigate(["/alterarCadastro/7"]);
    }
